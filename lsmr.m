function [x, istop, itn, normr, normAr, normA, cgitn, xitn]...
...
%-----------------------------------------------------------------------
  ...
  pfreq  = 100;   % print frequency (for repeating the heading)
  ...
  if nargin < 4 || isempty(atol)     , atol      = 1e-10;       end
  if nargin < 5 || isempty(btol)     , btol      = 1e-10;       end
  if nargin < 6 || isempty(conlim)   , conlim    = 1e+8;       end
  if nargin < 7 || isempty(itnlim)   , itnlim    = minDim*10000;     end
  ...
  while itn < itnlim
    ...
    xitn(itn,:) = x;
%     figure(31)
%     plot(itn,x,'.k'); hold on
    
    h         = v - (thetanew/rho)*h;
cgitn(itn,:) = (zeta/(rho*rhobar))*hbar;
    % Estimate of ||r||.
    ...
    if show
      prnt = 0;
      if n     <= 40       , prnt = 1; end
      if itn   <= 100       , prnt = 1; end
      if itn   >= itnlim-10, prnt = 1; end
      if rem(itn,100) == 0  , prnt = 1; end
