function [data] = DataGenUniform(N,A,base)
...
if(doPlot.spectrum==1)
    [~] = MeanSpectrumPower( SIM.bg.data,setup.hertz );
end
% [SIM.bg.data] = ApplyResolution(SIM.bg.data, 128, 2);
% [SIM.bg.data] = BGSIM(SIM.bg.data,AQ.bg,setup.N);
% if(doPlot.spectrum==1)
%     [~] = MeanSpectrumPower( SIM.bg.data,setup.hertz );
% end
...
% dado = ApplyResolution(dado,10,2);
% figure
% plot(dado(:,33))
...
[AQ.sg] = doShape_Uniforme(AQ.sg,setup.amostras,F,doPlot.shape);
% SIMULAÇÃO DO SINAL
[SIM.sg.data,SIM.sg.A] = SGSIM_Uniforme(AQ.sg.phase,AQ.sg,N,setup.fitA,doPlot.fitA);

% figure
% teste = ApplyResolution(SIM.sg.data(:,1:10)+SIM.bg.data(:,1:10),10,2);
% plot(teste)
% pause
